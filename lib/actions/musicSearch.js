...
{   /// This function performs the search
  console.log(`ANTHO : service : ${service}, type: ${type}, term: ${term}`)
  var serviceDef = getService(service); // serviceDef should be equal to deezerDef, things ought to be super interesting ! 
  var url = serviceDef.search[type]; // url: https://api.deezer.com/search?limit=1&q=album:
  var authenticate = serviceDef.authenticate;

  term = decodeURIComponent(term); // What does this nigga do ? 
  console.log(`ANTHO : term ${term}`)

  // Check for search type specifiers
  // With this approach, you can only specify one type specifier ... 
  if (term.indexOf(':') > -1) {
    console.log("[DEBUG] Antho")
    var newTerm = '';
    var artistPos = term.indexOf('artist:'); // 0
    var albumPos  = term.indexOf('album:'); // 12
    var trackPos  = term.indexOf('track:'); // -1
    var nextPos = -1;
    var artist = '';
    var album = '';
    var track  = '';
    ...
    // ALBUM : ARTIST
    if (artistPos > -1) { // There's an artist
      nextPos = (albumPos < trackPos)?albumPos:trackPos; // -1
      artist = term.substring(artistPos+7,(artistPos < nextPos)?nextPos:term.length); // We extract until the end, so artist should be at the end of the query
    }
    if (albumPos > -1) { // There's an album
      nextPos = (trackPos < artistPos)?trackPos:artistPos;  // nextPos = -1
      album = term.substring(albumPos+6,(albumPos < nextPos)?nextPos:term.length); // We extract until the end, so it won't fucking wokr. 
    }
    if (trackPos > -1) { 
      nextPos = (albumPos < artistPos)?albumPos:artistPos; 
      track = term.substring(trackPos+6,(trackPos < nextPos)?nextPos:term.length);
    }
    console.log(`[DEBUG][MusicSearch.js] Antho : artist:${artist}, track:${track}, album:${album}`);
    newTerm = serviceDef.term(type, term, artist, album, track);
    console.log(`[DEBUG][MusicSearch.js] Antho : newTerm:${newTerm}`);
  ...
  //url += `"${newTerm}"`;  // https://api.deezer.com/search?limit=1&q=album:"<le new term>"
  url += newTerm
  console.log(`Antho : url ${url}`)
...
function musicSearch(player, values) { // Handler for music search method. 
